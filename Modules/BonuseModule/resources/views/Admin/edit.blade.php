...
<div class="content-wrapper container-fluid">
    ...
    <div class="content-header mb-2">
        <h3><i class="fa fa-edit"></i> &nbsp; تعديل مكافأة</h3>
    </div>
    ...
    <div class="card">
        <div class="card-content">
            <form class="card-form side-form" method="POST"
                  action="{{ route('admin.bonuses.update', $bonuse->id) }}">
                @csrf
                            ...
                            <select id="branchSelect" name="branch_id" class="form-control">
                                ...
                                @foreach($branches as $branch)
                                    <option value="{{ $branch->id }}"
                                        {{ old('branch_id', $bonuse->employee?->branch_id) == $branch->id ? 'selected' : '' }}>
                                        {{ $branch->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-lg-2 col-sm-12 col-6">
                        ...
                        <div class="form-group">
                            <input type="number" id="amount" name="amount"
                                   ...
                                   value="{{ old('amount', $bonuse->amount) }}">
                            @error('amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-lg-6 col-sm-12">
                        ...
                        <div class="form-group">
                            <textarea id="reason" name="reason" rows="3"
                                      class="form-control @error('reason') is-invalid @enderror">{{ old('reason', $bonuse->reason) }}</textarea>
                            @error('reason')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="col-12 mt-1">
                    <a href="{{ route('admin.bonuses.index', ['month' => $month]) }}"
...
(function () {
    ...
    var initialBranchId = "{{ old('branch_id', $bonuse->employee?->branch_id) }}";
    var initialDeptId   = "{{ old('department_id', $bonuse->employee?->department_id) }}";
    var initialEmpId    = "{{ old('employee_id', $bonuse->employee_id) }}";
